...
function getFlatMenuData(menus) {
    ...
    menus.forEach(item => {
        if (item.children) {
            keys[item.path] = Object.assign({}, item);
            keys = Object.assign({}, keys, getFlatMenuData(item.children));
        }
        else {
            keys[item.path] = Object.assign({}, item);
        }
...
export const getRouterData = () => {
    ...
    Object.keys(routerConfig).forEach(path => {
        ...
        router = Object.assign({}, router, { name: router.name || menuItem.name, authority: authority, meta: {
                authority: authority
            } });
